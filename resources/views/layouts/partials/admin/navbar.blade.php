        ...
        <div x-data="{ open: false, unreadCount: {{ auth()->user()->unreadNotifications->count() ?? 0 }} }" class="relative hidden md:block">
            <button @click="open = !open" type="button" class="relative p-2 text-gray-400 hover:text-gray-600 rounded-full hover:bg-gray-100 focus:outline-none">
                ...
                <template x-if="unreadCount > 0">
                    <span
                        :class="(unreadCount > 99) ? 'absolute -top-2 -right-1 bg-red-500 text-white font-bold rounded-full flex items-center justify-center min-w-[1.75rem] min-h-[1.75rem] py-1 border-2 border-white shadow-md' : 'absolute -top-2 -right-2 bg-red-500 text-white font-bold rounded-full flex items-center justify-center min-w-[1.75rem] min-h-[1.75rem] px-1.5 py-1 border-2 border-white shadow-md'"
                        :style="'letter-spacing:0.2px; font-size:0.65rem;'"
                    >
