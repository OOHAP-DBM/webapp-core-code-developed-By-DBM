...
<div class="py-4 px-2 sm:px-4 md:px-6 lg:px-8 w-full max-w-8xl mx-auto">
           ...
           <div class="w-full max-w-5xl mx-auto py-6">
            ...
            <form action="{{ route('vendor.hoarding.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                @elseif($step == 2)
                    @include('hoardings.vendor.partials.step2', ['draft' => $draft])
                @elseif($step == 3)
                ...
                <div class="flex justify-between gap-3 mb-8">
                    ...
                    <button type="submit" class="px-7 py-2 rounded bg-green-600 text-white font-semibold shadow hover:bg-green-700 transition" style="cursor:pointer;">
                        @if($step < 3)
                            Next
                        @else
                            Publish
