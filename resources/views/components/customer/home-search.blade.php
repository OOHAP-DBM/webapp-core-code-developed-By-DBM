<div class="hidden md:flex items-center flex-1 max-w-3xl mx-4 bg-[#f0f0f0] rounded">
                <form action="{{ route('search.seo', ['city' => request('location', 'india'), 'area' => request('area')]) }}" method="GET" class="flex items-center w-full bg-[#f0f0f0] rounded border border-white overflow-hidden">
